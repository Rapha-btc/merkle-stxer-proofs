...
exports.bitcoinTxProof = bitcoinTxProof;
...
function extractBlockHeader(blockHex) {
    return blockHex.substring(0, 160);
}
function getTxHashes(block) {
    debug('Getting tx hashes from:', block.tx);
    const hashes = block.tx.map((tx) => {
...
function verifyMerkleProof(txid, proofSteps, merkleRoot) {
    ...
    for (const step of proofSteps) {
        ...
        currentHash = (0, merkle_1.hash256)(combined);
...
async function bitcoinTxProof(txid, blockHeight, rpcConfig) {
    const rpc = new rpc_1.BitcoinRPC(rpcConfig);
    ...
    const txIndex = block.tx.findIndex((tx) => tx.txid === txid);
    ...
    if (block.tx.length === 1) {
        ...
        merkleProof = (0, merkle_1.getMerkleProof)(txHashes, txIndex);
    ...
    if (block.tx.length > 1 && txIndex !== 0) {
        ...
        try {
            ...
            const txs = await Promise.all(block.tx.map(async (tx) => {
                const rawTxData = await rpc.call('getrawtransaction', [tx.txid, true, blockHash]);
                const hasWitness = rawTxData.hex.includes('0001') || rawTxData.vin.some((input) => input.txinwitness);
                debug(`Transaction ${tx.txid}:`, {
                    hasWitness,
                    vinLength: rawTxData.vin.length,
                    hasWitnessData: rawTxData.vin.some((input) => input.txinwitness)
                });
                return rawTxData.hex;
            }));
            debug('Calculating witness merkle proof...');
            const { proof, root } = (0, merkle_1.calculateWitnessMerkleProof)(txs, txIndex);
        ...
        debug('Skipping witness merkle proof:', block.tx.length === 1 ? 'single-tx block' :
