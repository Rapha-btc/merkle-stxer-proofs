...
export function exponentialBackoff(initDelay = 1000, maxDelay = 60_0000) {
    return (attempt) => {
        return Math.min(maxDelay, 2 ** attempt * initDelay);
    };
}
export function retryOnError(_attempt, error, response) {
    if (error != null)
        return true;
    if (response != null) {
...
export async function richFetch(url, options) {
